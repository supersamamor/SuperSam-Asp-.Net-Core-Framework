...
public class GetUnitQueryHandler : BaseQueryHandler<ApplicationContext, UnitState, GetUnitQuery>, IRequestHandler<GetUnitQuery, PagedListResponse<UnitState>>
{
	...
	public override async Task<PagedListResponse<UnitState>> Handle(GetUnitQuery request, CancellationToken cancellationToken = default) =>
		await Context.Set<UnitState>().Include(l=>l.Project)
