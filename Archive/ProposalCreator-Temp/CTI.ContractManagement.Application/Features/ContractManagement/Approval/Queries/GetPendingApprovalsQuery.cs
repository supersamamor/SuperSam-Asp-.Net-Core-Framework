...
namespace CTI.ContractManagement.Application.Features.ContractManagement.Approval.Queries;
...
public class GetPendingApprovalsQueryHandler : IRequestHandler<GetPendingApprovalsQuery, PagedListResponse<PendingApproval>>
{
    ...
    {
        string? recordName = "";
		if(tableName == ApprovalModule.Project)
		{
			recordName = context.Project.Where(l => l.Id == dataId).AsNoTracking().FirstOrDefault()?.DocumentCode;
		}
