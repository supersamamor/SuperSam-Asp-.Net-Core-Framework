...
namespace CompanyPL.ProjectPL.Scheduler.Jobs
{
    ...
    public class ApprovalNotificationJob : IJob
    {
        ...
        private string SetApprovalUrl(string message, ApprovalRecordState approvalRecord)
        {
            ...
            {
				message = message.Replace(EmailContentPlaceHolder.ApprovalUrl, $"{_baseUrl}/ProjectPL/{Core.Helpers.ConstantHelper.GetPropertyNameByValue(typeof(ApprovalModule), approvalRecord!.ApproverSetup!.TableName)}/Approve?Id={approvalRecord.DataId}");
            }
            return message;
        }
