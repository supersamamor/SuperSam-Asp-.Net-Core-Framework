...
using Quartz;
using CelerSoft.Common.Services.Shared.Interfaces;
using CelerSoft.Common.Services.Shared.Models.Mail;

namespace CelerSoft.TurboERP.Scheduler.Jobs
{
    public class ApprovalNotificationJob : IJob
    {
        ...
        private string SetApprovalUrl(string message, ApprovalRecordState approvalRecord)
        {
            ...
            {
                message = message.Replace(EmailContentPlaceHolder.ApprovalUrl, $"{_baseUrl}/TurboERP/{approvalRecord!.ApproverSetup!.TableName}/Approve?Id={approvalRecord.DataId}");
            }
            return message;
        }
