...
[Authorize(Policy = Permission.TaskList.Approve)]
public class ApproveModel : BasePageModel<ApproveModel>
{
    ...
    {
        return await TryThenRedirectToPage(async () => await Mediatr.Send(new ApproveCommand(TaskList.Id, "")), "Approve", true);
    }
    private async Task<IActionResult> Reject()
    {
        return await TryThenRedirectToPage(async () => await Mediatr.Send(new RejectCommand(TaskList.Id, "")), "Approve", true);
    }
}
